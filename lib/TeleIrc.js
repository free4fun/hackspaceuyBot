...
const IrcCommandHandler = require('./IrcHandlers/IrcCommandHandler');
...
const TgBlackList = require('./TelegramHandlers/TgListsHandler');
...
class TeleIrc {
  ...
  constructor(teleIrcConfig) {
    ...
    //Irc Stuff
    this.config.irc.MasterList = this.config.ircMasterlist;
    this.config.irc.MasterPass = this.config.ircMasterpass;
    this.config.irc.BlackList = this.config.ircBlacklist;
    // "Constants:"
  ...
  initStage1_ircConfigValidation() {
    let ircooptions = ["server", "channel", "botName", "botUsername", "botRealname"];
    if (this.config.irc) {
      ircooptions.forEach(option => {
        if (!this.config.irc[option]) {
          ...
          } else if (option === "botUsername") {
            errorCode = this.errorCodes.MissingIrcUsernameConfig;
          } else if (option === "botRealname") {
            errorCode = this.errorCodes.MissingIrcRealnameConfig;
          }
  ...
  nickservIdentify() {
    if ((this.config.irc.nickservPassword.length > 0) &&
      ...
      this.ircbot.say(this.config.irc.nickservService, "IDENTIFY " + nickservRegisteredNick + " " + this.config.irc.nickservPassword);
    } else {
  ...
  initStage4_addIrcListeners() {
    ...
    this.nickServHandler = new IrcNickServHandler(
      this.config.irc,
      this.ircbot
    );
    ...
    this.commandHandler = new IrcCommandHandler(
      this.sendMessageToIrc.bind(this),
      this.queueTelegramMessage.bind(this),
      this.config.irc,
      true
    );
    this.ircbot.addListener('message', this.commandHandler.GetCommand.bind(this.commandHandler));
    this.ircbot.addListener('nick', this.commandHandler.UserChangedNick.bind(this.commandHandler));
    this.ircbot.addListener('part', this.commandHandler.UserHasPart.bind(this.commandHandler));
    this.ircbot.addListener('quit', this.commandHandler.UserHasQuit.bind(this.commandHandler));
